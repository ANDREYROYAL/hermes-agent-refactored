        ...
        for hook_dir in sorted(HOOKS_DIR.iterdir()):
            ...
            try:
                ...
                if not manifest or not isinstance(manifest, dict):
                    logger.warning(f"Skipping hook {hook_dir.name}: invalid HOOK.yaml")
                    continue

                hook_name = manifest.get("name", hook_dir.name)
                events = manifest.get("events", [])
                if not events:
                    logger.warning(f"Skipping hook {hook_name}: no events declared")
                    continue
                ...
                if spec is None or spec.loader is None:
                    logger.warning(f"Skipping hook {hook_name}: could not load handler.py")
                    continue
                ...
                if handle_fn is None:
                    logger.warning(f"Skipping hook {hook_name}: no 'handle' function found")
                    continue
